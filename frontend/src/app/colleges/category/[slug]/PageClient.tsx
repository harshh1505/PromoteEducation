...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!query) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="text-center max-w-md">
        ...
        <Link href="/colleges" className="inline-block bg-sky-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-sky-700 transition-all">Explore All Colleges</Link>
      </div>
    </div>
  )
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center">
      <div className="text-slate-400 text-lg font-medium">Loading colleges...</div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-xl font-bold text-slate-900">{colleges.length} {query.stream} Colleges Found</h2>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      {college.nirf_rank && (
                        <span className="bg-amber-50 text-amber-600 text-xs font-bold px-2 py-1 rounded border border-amber-100 uppercase">NIRF #{college.nirf_rank}</span>
                      )}
                      <span className="bg-sky-50 text-sky-600 text-xs font-bold px-2 py-1 rounded border border-sky-100 uppercase">{college.stream}</span>
                    </div>
                    <h3 className="text-xl font-bold text-slate-900 group-hover:text-sky-600 transition-colors mb-2">{college.name}</h3>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-20 border-t border-slate-100 pt-16">
        <h2 className="text-xl font-bold mb-8 text-slate-900">Explore by Location</h2>
        <div className="flex flex-wrap gap-x-12 gap-y-4">
          {['delhi', 'mumbai', 'bangalore', 'pune'].map(city => (
            <Link key={city} href={`/colleges/category/${query.stream.replace(/\s+/g, '-').toLowerCase()}-in-${city}`}
              className="text-sky-600 hover:text-sky-700 font-bold text-sm transition-colors uppercase tracking-wide">
              {query.stream} in {city}
            </Link>
          ))}
        </div>
      </section>
